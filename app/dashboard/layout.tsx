...
const navigation = [
  ...
  { name: 'Mi Negocio', href: '/dashboard/negocio', icon: MapPin },
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/login')
    } else if (!isLoading && !hasRole(['owner', 'admin'])) {
      router.push('/')
    }
  ...
  if (!isAuthenticated || !hasRole(['owner', 'admin'])) {
    return null
  }
        ...
        <div className="flex h-16 items-center justify-between px-4">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold">
              Zona<span className="text-sidebar-primary">Local</span>
            </span>
          </Link>
          <Button
            variant="ghost"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="truncate text-xs text-sidebar-foreground/60">
                {user?.role === 'owner' ? 'Dueño de negocio' : 'Administrador'}
              </p>
            </div>
          </div>
        </div>
